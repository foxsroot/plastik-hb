...
describe('contactController', () => {
  ...
  it('getContactInfo returns contact info', async () => {
    (ContactService.getContactSection as jest.Mock).mockResolvedValue({ id: '1', type: 'ADDRESS' });
  ...
  it('putContactInfo updates contact info', async () => {
    (ContactService.updateContactSection as jest.Mock).mockResolvedValue({ id: '1', type: 'ADDRESS' });
    mockReq.body = { phoneNumber: '123' };
    const result = await contactController.putContactInfo(mockReq, mockRes);
    expect(result).toEqual({ data: { id: '1', type: 'ADDRESS' }, status: 200, message: 'Contact info updated' });
  });
});