...
const transformApiProductToFrontend = (apiProduct: any) => {
  ...
  const discountedPrice =
    discount > 0
      ? Math.max(
          0,
          Math.round(apiProduct.price - (apiProduct.price * discount) / 100),
        )
      : 0;
  ...
  return {
    ...
    images:
      additionalImages.map((asset: any) => getImageOrAlt(asset.url)) || [],
...
const priceRules = [
  ...
  (v: string) =>
    validatePrice(v) || "Harga harus berupa angka yang valid dan lebih dari 0",
  (v: string) => {
...
const modalTitle = computed(() =>
  isEditMode.value ? "Edit Produk" : "Tambah Produk Baru",
);
...
const handleImageUpload = async (event: Event) => {
  ...
  if (file) {
    if (
      ...
      uploadedFiles.value = [
        file,
        ...uploadedFiles.value.filter((_, index) => index !== 0),
      ];
    } else {
...
const handleAdditionalImageUpload = (event: Event) => {
  ...
  if (files && files.length > 0) {
    ...
    console.log(
      `Current additional images: ${currentAdditionalImages}, Available slots: ${availableSlots}`,
    );
    ...
    if (filesToProcess.length < files.length) {
      error(
        "Upload Terbatas",
        `Hanya ${filesToProcess.length} gambar yang dapat ditambahkan. Maksimal 7 gambar tambahan.`,
      );
    }
...
const removeAdditionalImage = (index: number) => {
  if (newProduct.value.images) {
    ...
    console.log(
      "Additional assets:",
      additionalAssets.map((a) => `${a.id} (order: ${a.order})`),
    );
    ...
    if (index < additionalAssets.length) {
      ...
      if (assetToDelete && !assetsToDelete.value.includes(assetToDelete.id)) {
        ...
        console.log(
          `Marked asset ${assetToDelete.id} (order: ${assetToDelete.order}) for deletion`,
        );

        // Remove from existingAssets tracking array immediately to prevent double processing
        const assetIndex = existingAssets.value.findIndex(
      ...
      if (isEditMode.value) {
        ...
        if (
          newFileIndex >= 0 &&
          newFileIndex < additionalImageFiles.value.length
        ) {
    ...
    console.log(
      `Removed from UI, new images length: ${newProduct.value.images.length}`,
    );
  }
};
...
const loadProductData = () => {
  if (editingProduct.value) {
    ...
    console.log(
      "Sorted assets:",
      sortedAssets.map((a) => `${a.id} (order: ${a.order})`),
    );
    console.log(
      "Additional assets:",
      additionalAssets.map((a) => `${a.id} (order: ${a.order})`),
...
const openEditModal = async (product: Product) => {
  ...
  try {
    ...
    if (updatedProduct) {
      ...
      originalAssets.value = updatedProduct.assets
...
const saveProduct = async () => {
  if (!newProduct.value.name || !newProduct.value.price) {
    showAlert(
      "error",
      "Validasi Error",
      "Mohon lengkapi data produk yang wajib diisi (Nama Produk dan Harga)",
    );
    return;
  }
    ...
    if (isEditMode.value && editingProduct.value) {
      ...
      try {
        ...
        if (isReplacingMainImage.value && pendingMainImageFile.value) {
          await replaceMainImageAPI(
            editingProduct.value.id,
            pendingMainImageFile.value,
          );

          // After replacing main image, refresh product data to get updated assets
          await fetchProducts();
          const refreshedProduct = products.value.find(
            (p) => p.id === editingProduct.value!.id,
          );
          if (refreshedProduct) {
            editingProduct.value = { ...refreshedProduct };
            existingAssets.value = refreshedProduct.assets
              ? [...refreshedProduct.assets]
              : [];
          }
        }
        ...
        for (const assetId of assetsToDelete.value) {
          try {
            ...
            const assetExists = editingProduct.value.assets?.some(
            ...
            if (assetExists) {
              ...
              console.log(
                `Asset ${assetId} no longer exists (possibly replaced), skipping delete`,
              );
            }
          } catch (error: any) {
            ...
            if (
              ...
              console.log(
                `Asset ${assetId} already removed or replaced, skipping`,
              );
            } else {
              console.warn(
        ...
        if (index !== -1) {
          ...
          const discountedPrice =
            discount > 0
              ? calculateDiscountedPrice(response.price, discount)
              : 0;
          ...
          products.value[index] = {
            ...
            images:
              response.assets
                ?.slice(1)
                .map((asset: any) => getImageUrl(asset.url)) || [],
            discount: discount,
            discounted_price: discountedPrice,
          };
        }
      ...
      console.log(
        ...
        "Additional:",
        additionalImageFiles.value.length,
      );
      ...
      const discountedPrice =
        discount > 0
          ? calculateDiscountedPrice(savedProduct.price, discount)
          : 0;
      ...
      showAlert(
        "success",
        "Berhasil",
        "Produk berhasil disimpan ke database dengan kategori dan aset!",
      );
    }
  ...
  <v-container class="pa-6">
                ...
                <v-row class="pa-4 align-center" no-gutters>
                  ...
                  <v-col cols="4" md="5" class="pl-4">
                    ...
                    >
                      ...
                      <!-- Harga Asli (Dicoret) -->
                      <p
                        class="text-body-2 text-grey text-decoration-line-through mb-0"
                      >
                  ...
                  <v-col cols="3" md="3" class="text-center">
                    <v-chip
                      ...
                      size="default"
                      :prepend-icon="
                        product.status === 'active' ? 'mdi-check' : 'mdi-clock'
                      "
                    >
            ...
            <v-row>
                  ...
                  <div class="additional-images-section">
                    ...
                    <div class="additional-images-container mb-3">
                      <div class="images-slider-wrapper">
                        <div class="images-slider" ref="imagesSlider">
                          <!-- Existing Images -->
                          <div
                  ...
                  <v-col cols="12" md="6">
                    ...
                    >
                      <v-chip color="success" variant="tonal" size="small">
                        ...
                        Harga Setelah Discount:
                        {{ formatPrice(previewDiscountedPrice || 0) }}
                      </v-chip>
                    </div>
                  </v-col>
              ...
              <v-col cols="12">
                <v-card variant="outlined" class="category-section">
                  <v-card-title
                    class="text-subtitle-1 font-weight-medium py-3 px-4 bg-grey-lighten-5"
                  >
                  ...
                  <v-card-text class="pa-4">
                    <div class="d-flex align-center gap-3">
                      <v-select
                        ...
                        required
                        :error-messages="
                          !newProduct.category_id && formSubmitted
    ...
    >
      <v-card class="category-modal-card">
        <v-card-title
          class="d-flex align-center justify-space-between py-4 px-6 bg-primary"
        >
        ...
        <v-card-text class="pa-6">
          <v-form @submit.prevent="createCategory">
              ...
              <span class="text-body-2 text-grey-darken-1">
                {{
                  editingCategory
                    ...
                    : "Masukkan nama kategori produk baru yang ingin ditambahkan"
                }}
              </span>
            </div>
            ...
            <v-text-field
              ...
              required
              :error-messages="
                formSubmitted && !newCategoryName.trim()
        ...
        <v-card-actions class="px-6 pb-6">
          ...
          >
            <v-icon start>{{
              editingCategory ? "mdi-content-save" : "mdi-plus"
            }}</v-icon>
    ...
    >
      <v-card class="category-manager-card">
        <v-card-title
          class="d-flex align-center justify-space-between py-4 px-6 bg-info"
        >
          <div class="d-flex align-center">
